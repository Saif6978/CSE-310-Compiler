...
class SymbolTable{
    ...
    void exit_scope(){
        if(current->getParent()==nullptr || current->getParent() == nullptr){
            return;
        }
        ScopeTable* temp=current;
        current=current->getParent();
    ...
    void print_this_table(ostream &out = cout){
        if(current!=nullptr) current->print(out);
    }
    void print_all_tables(ostream &out = cout){
        ScopeTable* curr = current;
        while(curr != nullptr){
            out << endl << endl;
            curr->print(out);
