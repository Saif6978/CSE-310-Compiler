...
// SDBM Hash Function as specified in the assignment
...
class ScopeTable {
public:
    inline static int scope_count = 0;
...
public:
    ScopeTable(int number_of_buckets, ScopeTable* parent_scope = nullptr, int scope_id = -1) {
        if (scope_id > 0) {
            this->id = to_string(scope_id);
            if (scope_id > scope_count) scope_count = scope_id;
        } else {
    ...
    bool Insert(const string& name, const string& type, bool verbose = false) {
        ...
        if (existing == nullptr) {
            ...
            if (verbose) {
                cout << "\tInserted in ScopeTable# " << id << " at position " << bucket + 1 << ", " << chain_id + 1 << endl;
            }
            return true;
        } else {
            return false;
    ...
    // Specification explicitly names this operation Delete
