...
void AsmGenerator::allocateGlobalVariable(string name , string value){
    writeData(name + " dd " + value);
...
any AsmGenerator::visitDeclaration_list(CSubsetParser::Declaration_listContext *ctx){
    ...
    if(!ctx->ID().empty()){
        ...
        if(insideFunction){
            ...
            string value="0";
            if(ctx->CONST_INT()!=nullptr) value=ctx->CONST_INT()->getText();
            allocateGlobalVariable(name , value);
        }
    }
    return nullptr;
}
...
any AsmGenerator::visitStatement( CSubsetParser::StatementContext *ctx){
    if(ctx->PRINTLN()!=nullptr){
        ...
        return nullptr;
        */
        if(ctx->chain_print()!=nullptr){
            visit(ctx->chain_print());
        }
        if(ctx->expression_list()!=nullptr){
...
any AsmGenerator::visitExpression_list(CSubsetParser::Expression_listContext *ctx){
    if(ctx->expression_list()!=nullptr){
        visit(ctx->expression_list());
    }
...
any AsmGenerator::visitChain_print(CSubsetParser::Chain_printContext *ctx){
